...
def main():
    ...
    # if needs to test multiple frequencies, exp: freqs = [10, 33, 50, 100, 200]
    freq = 3
    ...
    bp_mean[freq] = []
    # directory name
    dir_name = "final/recruitment/0/" + str(freq) + '/' +  str(pud) + '-' + str(pel)+'/'
    ...
    for i in range(1):
        ...
        """
        simrun = bladderModel("frwSimCat.txt", freq, amp, 6000, 6000, 6000, 1, 0, 35.0, dir_name)
        simrun.createNetwork()

        # record the average bp/auc of pre/post stimulation during simulation
        bp_mean[freq].append((simrun.bp_pre, simrun.bp_post, simrun.bp_diff))


    try:
        # example directory: results/stim_freq/pud-pel/filename
        res_file = open('../../results/' + dir_name + "%.1f pud-%.1f pel at %.1f.txt" % (pud, pel, freq),'wt')
