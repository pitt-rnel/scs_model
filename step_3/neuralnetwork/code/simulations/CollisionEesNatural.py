...
class CollisionEesNatural(Simulation):
	...
	def plot(self,delay,nColorLevels=None,name=""):
		...
		if rank == 0:
			...
			if nColorLevels is not None:
				dataToPlot=old_div(np.round(old_div(dataToPlot,100)*nColorLevels)*100,nColorLevels)
			...
			plt.yticks(list(range(len(self._eesFrequencies))),self._eesFrequencies)
			plt.xticks(list(range(len(self._fiberFiringRates))),self._fiberFiringRates)
	...
	def plot_isoinformation_surface(self,percentage=50):
		...
		if rank==0:
			...
			plt.xticks(list(range(len(self._fiberDelays))),self._fiberDelays)
			plt.yticks(list(range(len(self._fiberFiringRates))),self._fiberFiringRates)
