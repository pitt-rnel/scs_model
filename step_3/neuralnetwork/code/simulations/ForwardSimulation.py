...
class ForwardSimulation(Simulation):
    ...
    def __init__(self, parallelContext, neuralNetwork, stim_end, afferentInput=None, eesObject=None, eesModulation=None,
        ...
        if afferentInput == None:
            ...
            key = []
            key.append(list(self._afferentInput.keys())[0])
            key.append(list(self._afferentInput[key[0]].keys())[0])
            ...
            if h.t > self.stim_end and h.t - self._timeUpdateEES >= (self._eesParam['dt'] - 0.5 * self._get_integration_step()):
                ...
                for muscle in self._eesParam['modulation']:
                    if self._eesParam['state'][muscle] != self._eesParam['modulation'][muscle][ind]:
                        if self._eesParam['state'][muscle] == 0:
                            self._ees.set_amplitude(self._eesParam['amp'], [muscle])
                        else:
    ...
    def _extract_results(self, samplingRate=1000.):
        ...
        for muscle in self._nn.actionPotentials:
            ...
            for cell in self._nn.actionPotentials[muscle]:
                self._firings[muscle][cell] = tlsf.exctract_firings(self._nn.actionPotentials[muscle][cell],
                                                                    self._get_tstop(), samplingRate)
                if rank == 0: self._nActiveCells[muscle][cell] = np.count_nonzero(
                    np.sum(self._firings[muscle][cell], axis=1))
